...
**近接時の表示**  
カメラが建物に近づくと、その付近のポリゴンだけ半透明・ワイヤーフレーム・市松模様で表示され、埋まっても周囲が見えるようになります。Debug パネルで表示モード（通常／AABB最近点／中心距離・巨大対策）を切り替えられます。

**Debug パネル**  
右上の Debug にチェックを入れると、FPS 表示と表示モード用のラジオボタン（通常・AABB最近点・中心距離）が表示されます。
