...
自動で前進する3Dの街を飛び回り、敵と戦いながらマスクを集めて成長するゲームです。
...
| **Q** | 上昇のみ |
| **E** | 下降のみ |
| **Wダブルタップ** | 宙返り（180度ターン、回避状態、エネルギー20消費） |
| **Sダブルタップ** | バレルロール回避（回避状態、エネルギー15消費） |
...
|------|------|
| 左ジョイスティック | 左右で旋回 |
| 右ジョイスティック | 上で加速+上昇、下で減速+下降 |

### ゲームシステム

- **自動前進**: 常に前方に移動し続けます
- **自動攻撃**: 敵に近づくと自動で攻撃します
- **マスク収集**: 敵を倒すとマスクがドロップ、近づくと回収
- **食べ物**: 黄色い球に近づくとエネルギー回復
- **輪廻転生**: エネルギーが0になると転生、マスクを引き継いで再開
...
**Debug パネル**  
右上の Debug ボタンをクリックすると、FPS 表示とデバッグオプションが表示されます。時間倍速やポーズ機能もここから使えます。
